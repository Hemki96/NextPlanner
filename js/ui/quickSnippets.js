...
const COLLAPSED_LAYOUT_CLASS = "layout--quick-collapsed";
...
export function initQuickSnippets({
  ...
  if (!toggleButton || !panel || !layout) {
    return;
  }

  const updateCollapsedState = (collapsed) => {
    layout.classList.toggle(COLLAPSED_LAYOUT_CLASS, collapsed);
    panel.dataset.quickCollapsed = collapsed ? "true" : "false";
    container.hidden = collapsed;
    container.setAttribute("aria-hidden", collapsed ? "true" : "false");
    toggleButton.setAttribute("aria-expanded", String(!collapsed));
    const label = collapsed
      ? "Schnellbausteine anzeigen"
      : "Schnellbausteine ausblenden";
    toggleButton.textContent = label;
    toggleButton.setAttribute("title", label);
  };

  toggleButton.addEventListener("click", () => {
    const collapsed = layout.classList.contains(COLLAPSED_LAYOUT_CLASS);
    const nextState = !collapsed;
    updateCollapsedState(nextState);

    if (!nextState) {
      const firstSnippetButton = container.querySelector(
        ".quick-snippet-button"
      );
      if (firstSnippetButton instanceof HTMLButtonElement) {
        firstSnippetButton.focus({ preventScroll: true });
      }
    }
  });
