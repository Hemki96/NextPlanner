...
- **DEV-Profil ohne Login:** Setze `NEXTPLANNER_ENV=dev`, um den Login-Bildschirm zu überspringen und im Browser direkt zwischen den Standardnutzern **admin**, **coach** und **athlete** zu wechseln. Das Passwort ist in diesem Profil immer `Test123`. In produktiven Umgebungen sollte das Profil nicht aktiviert werden.
- **Schnellstart DEV-Profil:** Mit `npm run dev:env` wird der Server im DEV-Profil mit den Default-Zugangsdaten (`Test123`) für admin, coach und athlete gestartet.
