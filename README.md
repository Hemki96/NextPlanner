...
- **Wo liegen die lokalen Datendateien?** – Die Anwendung legt `data/plans.json` (Pläne), `data/quick-snippets.json` (persönliche Schnellbausteine) und `data/team-snippets.json` (Team-Bibliothek) bei Bedarf automatisch an; alle Pfade sind in `.gitignore` ausgeschlossen. Die Snippet-Dateien enthalten immer ein gültiges `{ updatedAt, groups }`-Objekt und werden beim Laden automatisch saniert.
...
- **DEV-Profil ohne Login:** Setze `NEXTPLANNER_ENV=dev`, um den Login-Bildschirm zu überspringen und im Browser direkt zwischen den Standardnutzern **admin**, **coach** und **athlete** zu wechseln. Das Passwort ist in diesem Profil immer `DevPass123!`. In produktiven Umgebungen sollte das Profil nicht aktiviert werden.
- **Schnellstart DEV-Profil:** Mit `npm run dev:env` wird der Server im DEV-Profil mit den Default-Zugangsdaten (`DevPass123!`) für admin, coach und athlete gestartet. Wenn keine Default-User erzeugt werden sollen (z. B. in Produktion oder Audits), setze `NEXTPLANNER_DISABLE_DEFAULT_USERS=true`; dann werden keine Standardkonten angelegt und die Dev-Auth ist deaktiviert.
...
- **Deployment auf Render:** Wähle beim Anlegen einen *Web Service*, definiere das Startkommando `npm start` und füge eine Persistent Disk (≥1 GB) hinzu, die auf `/data` gemountet wird. Hinterlege `NEXTPLANNER_DATA_DIR=/data` in den Environment-Variablen, damit die JSON-Stores direkt auf das Volume schreiben können. Alternativ kannst du das mitgelieferte Blueprint [`render.yaml`](render.yaml) verwenden; es setzt `NODE_ENV=production`, mountet `/data` und generiert starke Passwörter für die Standardnutzer.
...
ist vom Repository ausgeschlossen). Eigene Schnellbausteine landen automatisch in `data/quick-snippets.json`, die Team-Snippet-
Bibliothek erzeugt zusätzlich eine `data/team-snippets.json`, sobald sie mit eigenen Gruppen befüllt wird.
